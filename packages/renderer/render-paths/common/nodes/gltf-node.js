...
export class GLTFNode extends RenderNode {
    ...
    attachments = {
        colors: {
            color:  new ColorAttachment({ format: 'rgba8unorm'  }),                       // 8 bytes aligned
            accum:  new ColorAttachment({ format: 'rgba16float' }),                       // 8 bytes aligned
            reveal: new ColorAttachment({ format: 'r8unorm', clearValue: [1, 1, 1, 1] }), // 1 bytes aligned (seems to be 4 in chrome)
            point:  new ColorAttachment({ format: 'rgba32float' }),                       //16 bytes aligned
            id:     new ColorAttachment({ format: 'r32uint'     }),                       // 4 bytes aligned
            motion: new ColorAttachment({ format: 'rg16float'   }),                       // 4 bytes aligned
        },
