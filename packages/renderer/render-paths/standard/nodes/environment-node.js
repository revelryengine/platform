...
import { RenderNode, CubeRenderNode, ColorAttachment } from '../../render-node.js';
...
import { float16, NonNull, pad3ChannelFormat, WeakCache } from 'revelryengine/utils/utils.js';
import { vec3                                           } from 'revelryengine/deps/gl-matrix.js';


const ENVIRONMENT_SIZE = 1024;
...
class LUTNode extends RenderNode {
    attachments = {
        colors: {
            color: new ColorAttachment({ format: 'rgba16float' }),
        },
    };
...
class EnvironmentResampleNode extends CubeRenderNode {
    attachments = {
        colors: {
            color: new ColorAttachment({ format: 'rgba16float', mipLevelCount: ENVIRONMENT_MIPLEVEL_COUNT }),
        }
    }
     ...
     * @param {NonNullable<import('../../../graph.js').Graph['environment']>} environment
     */
    ...
    reconfigure() {
        ...
        const texture       = this.gal.createTextureWithData({ format, size, mipLevelCount, usage, glCubemap: true, data });
        ...
        this.mipmapGenerator = this.gal.createMipmapGenerator(NonNull(this.output.color?.texture), 'cube');
     ...
     */
    render(renderPassEncoder) {
        this.shader?.run(renderPassEncoder, this.currentLayer);
    }
...
class EnvironmentPrefilterNode extends RenderNode {
    /**
     * @type {import('../../../revgal.js').REVBindGroup[]}
     */
    roughnessBindGroups = [];

    /**
     * @param {import('../../render-path.js').RenderPath} renderPath
     * @param {import('../../../graph.js').Graph['environment']} environment
     * @param {EnvironmentResampleNode} resample
     * @param {Record<string, import('../../../revgal.js').REVTexture>} textures
     */
    ...
    reconfigure() {
        ...
        this.charlieShader = new PrefilterShader(this.gal, { format: 'rgba16float', distribution: BRDF_DISTRIBUTIONS['charlie'], sampleCount: PREFILTER_SAMPLE_SIZE }).compileAsync();

        const sampler = this.gal.device.createSampler({ magFilter: 'linear', minFilter: 'linear', mipmapFilter: 'linear' });
        const view    = NonNull(this.resample.output.color?.texture).createView({ dimension: 'cube' });
        ...
        this.roughnessBindGroups.length = 0;
    ...
    /**
     * @param {Parameters<RenderNode['begin']>[0]} commandEncoder
     * @param {{ face: number, distribution: 'ggx'|'charlie', level: number }} data
     */
    ...
    /**
     * @param {Parameters<RenderNode['render']>[0]} renderPassEncoder
     * @param {{ face: number, distribution: 'ggx'|'charlie', level: number }} data
     */
    render(renderPassEncoder, { face, distribution, level }) {
        renderPassEncoder.setBindGroup(0, NonNull(this.bindGroup));
     ...
     */
    run(commandEncoder) {
        for(const distribution of /** @type {(keyof BRDF_DISTRIBUTIONS)[]} */(Object.keys(BRDF_DISTRIBUTIONS))) {
...
export class EnvironmentNode extends CubeRenderNode {
    attachments = {
        colors: {
            envGGX:     new ColorAttachment({ format: 'rgba16float', mipLevelCount: ENVIRONMENT_MIPLEVEL_COUNT }),
            envCharlie: new ColorAttachment({ format: 'rgba16float', mipLevelCount: ENVIRONMENT_MIPLEVEL_COUNT }),
        }
    }

    size = { width: ENVIRONMENT_SIZE, height: ENVIRONMENT_SIZE };

    lutNode = new LUTNode(this.renderPath);
    ...
    reconfigure() {
        ...
        this.output.envLUT      = NonNull(this.lutNode.output.color);
    }
    ...
    async #deriveIrradianceCoefficients(resample) {
        ...
        const read  = await this.gal.readTexture(NonNull(resample.output.color?.texture), {
    ...
    async #prefilterEnvironment(environment) {
        ...
        if(environment.cubemap) {
            ...
            cache.prefilter ??= new EnvironmentPrefilterNode(this.renderPath, environment, cache.resample, {
                ggx:     NonNull(this.output.envGGX?.texture),
                charlie: NonNull(this.output.envCharlie?.texture),
            }).reconfigure();
...
export function deriveIrradianceCoefficients(data, size) {
    const coefficients = /** @type {NonNullable<import('../../../graph.js').Graph['environment']>['irradianceCoefficients']} */(/** @type {unknown} */([...new Array(9)].map(() => new Float32Array(3))));
