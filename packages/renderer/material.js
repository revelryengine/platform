...
import { Material as GLTFMaterial } from 'revelryengine/gltf/gltf.js';


/**
 * @typedef {import('revelryengine/gltf/gltf.js').TextureInfo | import('revelryengine/gltf/gltf.js').MaterialNormalTextureInfo | import('revelryengine/gltf/gltf.js').MaterialOcclusionTextureInfo} TextureInfo
 * @typedef {{
...
/**
 * @template {keyof Revelry.GLTF.Extensions.Material} K
 * @typedef {keyof NonNullable<Revelry.GLTF.Extensions.Material[K]> & `${string}Texture`} MaterialExtensionTextureProperty
 */

/**
 * @typedef {{
 *  [K in keyof Revelry.GLTF.Extensions.Material as `${K}.${MaterialExtensionTextureProperty<K>}`]?: import('revelryengine/gltf/gltf.js').TextureInfo
 * } & {
 *  normalTexture?:            import('revelryengine/gltf/gltf.js').MaterialNormalTextureInfo,
 *  occlusionTexture?:         import('revelryengine/gltf/gltf.js').MaterialOcclusionTextureInfo,
 *  emissiveTexture?:          import('revelryengine/gltf/gltf.js').TextureInfo,
 *  baseColorTexture?:         import('revelryengine/gltf/gltf.js').TextureInfo,
 *  metallicRoughnessTexture?: import('revelryengine/gltf/gltf.js').TextureInfo,
 * }} TexturesList
 */

const STANDARD_TEXTURES = ['normalTexture', 'occlusionTexture', 'emissiveTexture', 'baseColorTexture', 'metallicRoughnessTexture'];
...
export class Material extends UBO.Layout({
    ...
    constructor(gal, material) {
        ...
        const {
            pbrMetallicRoughness = /** @type {import('revelryengine/gltf/gltf.js').MaterialPBRMetallicRoughness} */({}),
            normalTexture, occlusionTexture, emissiveTexture
        } = material;

        const { baseColorTexture, metallicRoughnessTexture } = pbrMetallicRoughness;

        this.textures   = /** @type {TexturesList} */({ normalTexture, occlusionTexture, emissiveTexture, baseColorTexture, metallicRoughnessTexture });
    ...
    upload() {
        ...
        const {
            pbrMetallicRoughness = /** @type {import('revelryengine/gltf/gltf.js').MaterialPBRMetallicRoughness} */({}),
            normalTexture, occlusionTexture, emissiveFactor, alphaCutoff
        } = material;
        ...
        for (const n in this.textures) {
            if(!STANDARD_TEXTURES.includes(n)) continue;
                ...
                for(const texName in ext) {
                    if(texName.endsWith('Texture')){
                        ...
                        if(texture) {
                            ...
                            this.textures[/** @type {keyof TexturesList & `KHR_${string}`} */(`${extName}.${texName}`)] = texture;
                        }
                    }
                }
            }
