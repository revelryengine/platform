...
import { roundUp } from 'revelryengine/utils/utils.js';
...
const generators = /** @type {const} */({
    'wgsl': {
        /**
         * @param {{ type: string, name: string, size: number, align: number, count?: number }} options
         */
        uniform: ({ type, name, size, align, count = 1 }) => {
            return `@size(${size * count}) @align(${align}) ${name}: ${count > 1 ? `array<${type}, ${count}>`: type},`;
        },
        /**
         * @param {{ type: string, layout: STD140Layout }} options
         */
        struct: ({ type, layout: { uniforms } }) => {
            return `struct ${type} {\n${uniforms.map(generators.wgsl.uniform).join(`\n`)}\n};`;
        },
        /**
         * @param {{ layout: STD140Layout, name: string }} ubo
         * @param {number|string} group
         * @param {number|string} binding
         * @param {string} [name]
         */
        block: (ubo, group, binding, name = ubo.name) => {
            const { layout } = ubo;
            const { structs, uniforms } = layout;

            const code = /* wgsl */`
                ${structs.map(generators.wgsl.struct).join('\n')}
    ...
    'glsl': {
        /**
         * @param {{ glsl: string, name: string, count?: number }} options
         */
        uniform: ({ glsl, name, count = 1 }) => {
         ...
         */
        struct: ({ type, layout: { uniforms } }) => {
            return `struct ${type} {\n${uniforms.map(generators.glsl.uniform).join(`\n`)}\n};`;
        },
        /**
         * @param {{ layout: STD140Layout, name: string }} ubo
         * @param {number|string} group
        ...
        block: (ubo, group, binding, name = ubo.name) => {
            ...
            const code = /* glsl */`
                ${structs.map(generators.glsl.struct).join('\n')}
...
export class UBO {
    ...
    constructor(gal, values) {
        this.gal    = gal;
        this.buffer = gal.device.createBuffer({ label: `${this.constructor.name}`, ...this.layout });
        this.data   = new Uint8Array(this.layout.size);

        this.layout.createView(this, this.data.buffer);

        /** @type {UBO & STD140LayoutView} */(this).set(Object.assign({}, this.defaults, values));
    }

    set = setViewValues;

    get name() {
        return 'structName' in this.constructor ? /** @type {string} */(this.constructor.structName) : this.constructor.name;
    }
     ...
     * @param {STD140LayoutValueSetter} [defaults]
     * @param {string} [name]
     */
    static Layout(layout, defaults, name) {
    ...
    static layout = new STD140Layout({});

    /**
     * @param {'wgsl'|'glsl'} type
     * @param {number|string} group
     * @param {number|string} binding
     * @param {string} [name]
     */
    static generateUniformBlock(type, group, binding, name) {
        const generator = generators[type];
        return generator.block(this, group, binding, name);
    }
     ...
     * @param {string} [name]
     */
    generateUniformBlock(group, binding, name) {
        const generator = generators[this.gal.language];
        return generator.block(this, group, binding, name);
    }
}
