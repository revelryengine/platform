...
export class RevGPU extends RevGAL {
     ...
     * @param {{ origin?: GPUOrigin3D, size?: GPUExtent3DDict, mipLevel?: number }} [options]
     */
    ...
    /**
     * @param {GPUTexture} texture
     * @param {'2d'|'2d-array'|'cube'} [viewDimension]
     */
     ...
     *
     * @param {import('./render-paths/render-node.js').DepthAttachment} attachment
     */
    createDepthTextureResolver(attachment) {
        const srcView = /** @type {GPUTextureView} */(NonNull(attachment.unresolved).createView());
        const dstView = /** @type {GPUTextureView} */(NonNull(attachment.texture).createView());

        const shader = new ResampleShader(this, {
            label:         'createDepthTextureResolver',
            view:          srcView,
