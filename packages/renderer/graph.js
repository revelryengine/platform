import { vec3, mat3, mat4           } from 'revelryengine/deps/gl-matrix.js';
import { BUFFER_USAGE, SHADER_STAGE } from './constants.js';

import { Material           } from './material.js';
import { MatrixTBO, TBO     } from './tbo.js';
import { NonNull, WeakCache, roundUp } from 'revelryengine/utils/utils.js';

class ModelMatrixTBO extends MatrixTBO.Layout({ width: 64, height: 64, limit: 8, double: true }) {}

class JointMatrixTBO extends MatrixTBO.Layout({ width: 64, height: 64, limit: 8, double: true }) {}

class MorphMatrixTBO extends MatrixTBO.Layout({ width: 64, height: 64, limit: 8, double: true }) {}

class GameObjectTBO extends TBO.Layout({ width: 64, height: 64, limit: 8 }) {}
...
export class Graph {
    state = {
        nodes:       /** @type {WeakMap<import('revelryengine/gltf/gltf.js').Node,NodeState> } */(new WeakMap()),
        meshes:      /** @type {WeakMap<import('revelryengine/gltf/gltf.js').Mesh, MeshState>} */(new WeakMap()),
        skins:       /** @type {WeakMap<import('revelryengine/gltf/gltf.js').Skin, SkinState> } */(new WeakMap()),
        morphs:      /** @type {WeakMap<number[], MorphState> } */(new WeakMap()),
        gameObjects: /** @type {WeakMap<import('revelryengine/gltf/gltf.js').NodeREVGameObject, GameObjectState> } */(new WeakMap()),
    }
     ...
     * @param {import('revelryengine/gltf/gltf.js').Scene} scene
     */
    ...
    /**
     * @param {import('./render-paths/render-path-settings.js').RenderPathSettings} settings
     */
    update(settings) {
     ...
     * @param {import('revelryengine/gltf/gltf.js').MeshPrimitive} primitive
     */
     ...
     * @param {import('revelryengine/gltf/gltf.js').Node} node - The node to get the state for
     */
    getNodeState(node) {
        let state = this.state.nodes.get(node); //weakmap is kind of slow, come back to this
        if (!state) {

            /** @type {MeshState|undefined} */
            let meshState;
            if(node.mesh) {
                ...
                meshState = {
                    ...
                    morph: node.mesh.weights ? this.getMorphState(node.mesh.weights) : undefined,

                    free: () => {
                        free();
                        meshState?.morph?.free();
                    }
                }
    ...
    /**
     * @param {import('revelryengine/gltf/gltf.js').Skin} skin
     */
    getSkinState(skin) {
    ...
    /**
     * @param {import('revelryengine/gltf/gltf.js').NodeREVGameObject} REV_game_object
     */
     ...
     * @param {import('revelryengine/gltf/gltf.js').Node} node
     * @return {mat4}
     */
     ...
     *
     * @param {import('revelryengine/gltf/gltf.js').Node} node - The node to update
     */
    updateNode(node) {
        ...
        if (parentState) {
            mat4.mul(worldTransform, parentState.worldTransform, localTransform);
        }
     ...
     *
     * @param {Set<import('revelryengine/gltf/gltf.js').Node>} nodes
     * @param {import('revelryengine/gltf/gltf.js').Node|import('revelryengine/gltf/gltf.js').Scene} root
     */
    updateNodes(nodes, root = this.scene) {
        const search = [...('nodes' in root ? root.nodes : root.children)];

        while (search.length) {
            const node = /** @type {import('revelryengine/gltf/gltf.js').Node} */(search.shift());
            if (nodes.has(node)) {
                this.updateNode(node);
            } else {
     ...
     *
     * @param {{ frustum: import('./frustum.js').Frustum, filter?: GameObjectFilter, sortAlpha?: boolean }} options
     */
    generateInstances({ frustum, filter, sortAlpha }) {
        ...
        const batches = {
            opaque:       Object.groupBy(this.instances.opaque,       byPrimitiveId),
            transmissive: Object.groupBy(this.instances.transmissive, byPrimitiveId),
            alpha:        Object.groupBy(this.instances.alpha,        byPrimitiveId),
            outline:      Object.groupBy(this.instances.outline,      byPrimitiveId),
        }
        ...
        for(const entry of Object.entries(blocks)) {
            ...
            for(const batch of Object.values(batches[type])) {
                ...
                block.batches.push({ offset: i / 4, count, primitive, frontFace, instances, depth: 0 });
                for(const { index, morph, skin, gameObject, worldTransform } of batch) {

                    instances.push(worldTransform);

                    /**
                     * When rendering game objects it's not generally useful to use instancing (such as with querying),
                     * so we can just store references to one of the other buffers and render them one at a time.
                     * we can skip the outline block because the instance should already be present in one of the other blocks.
                     */
        ...
        if(sortAlpha) {
            ...
            block.sorted = /** @type {InstanceBatch[]} */([]);
            ...
            for(const batch of block.batches) {
                ...
                for(const worldTransform of instances){
                    ...
                    block.sorted.push({ offset: offset + (i++ * 4), primitive, frontFace, depth: pos[2], count: 1, instances: [worldTransform] });
                }
    ...
    /** @type {WeakCache<Record<InstanceType, InstanceBlock> & Record<'alpha', InstanceBlock & { sorted?: InstanceBatch[]}>>} */
    ...
    #getInstanceBlocks(frustum) {
        const instanceBlocks = this.#instanceBlockCache.ensure(frustum, () => ({
            opaque       : { size: 0, batches: [], data: new Uint32Array(0), buffer: this.gal.device.createBuffer({ size: 0, usage: BUFFER_USAGE.VERTEX | BUFFER_USAGE.COPY_DST }) },
            transmissive : { size: 0, batches: [], data: new Uint32Array(0), buffer: this.gal.device.createBuffer({ size: 0, usage: BUFFER_USAGE.VERTEX | BUFFER_USAGE.COPY_DST }) },
            alpha        : { size: 0, batches: [], data: new Uint32Array(0), buffer: this.gal.device.createBuffer({ size: 0, usage: BUFFER_USAGE.VERTEX | BUFFER_USAGE.COPY_DST }) },
            outline      : { size: 0, batches: [], data: new Uint32Array(0), buffer: this.gal.device.createBuffer({ size: 0, usage: BUFFER_USAGE.VERTEX | BUFFER_USAGE.COPY_DST }) },
        }));
    ...
    /**
     * @param {import('./viewport.js').ViewportCameraNode} cameraNode
     */
    fitCameraToScene(cameraNode) {
        ...
        const yfov   = cameraNode.camera.getYFov();
    ...
    /**
     * @type {WeakMap<import('revelryengine/gltf/gltf.js').Material, Material>}
     */
    #materialUBOs = new WeakMap();
    getMaterialUBO(material = Material.DEFAULT_MATERIAL){
        return this.#materialUBOs.get(material) ?? NonNull(this.#materialUBOs.set(material, new Material(this.gal, material)).get(material));
    }
    ...
    /**
     * @type {import('revelryengine/gltf/gltf.js').KHRMaterialsVariantsVariant|null}
     */
    #activeVariant = null;

    /**
     * @param {import('revelryengine/gltf/gltf.js').KHRMaterialsVariantsVariant} variant
     */
    ...
    /**
     * @param {Set<import('revelryengine/gltf/gltf.js').Material>} materials
     */
    ...
    /**
     * @param {import('revelryengine/gltf/gltf.js').Material} material
     */
    ...
    /**
     * @param {{
     *  'revelryengine/gltf/gltf.js').Node>
     *  'revelryengine/gltf/gltf.js').Material>,
     * }} targets
     */
    ...
    deleteNode(node) {
        ...
        if(node.skin) this.skins.delete(/** @type {SkinNode}*/(node));
        if(node.mesh) this.morphs.delete(/** @type {MeshNode}*/(node));
        if(node.extensions?.KHR_lights_punctual) this.lights.delete(/** @type {LightNode}*/(node));
        if(node.extensions?.KHR_audio) this.emitters.delete(/** @type {EmitterNode}*/(node));
 ...
 * @typedef {{
*  index:          number,
*  node:           import('revelryengine/gltf/gltf.js').Node,
*  primitive:      import('revelryengine/gltf/gltf.js').MeshPrimitive,
...
* @typedef {{ offset: number, count: number, primitive: import('revelryengine/gltf/gltf.js').MeshPrimitive, frontFace: 'cw'|'ccw', instances: mat4[], depth: number }} InstanceBatch
* @typedef {{ size: number, batches: InstanceBatch[], buffer: import('./revgal.js').REVBuffer, data: Uint32Array }} InstanceBlock
* @typedef {{ buffer: import('revelryengine/gltf/gltf.js').MeshPrimitive, frontFace: 'cw'|'ccw' }} GameObjectInstanceBlock
*
...
* }} MeshState
* @typedef {{
*  parent:         import('revelryengine/gltf/gltf.js').Node|null,
*  localTransform: mat4,
*  worldTransform: mat4,
*  mesh?:          MeshState,
*  skin?:          SkinState,
