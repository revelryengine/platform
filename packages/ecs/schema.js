...
/**
 * @template {ComponentTypeSchema} [T=ComponentTypeSchema]
 */
export class ComponentSchemaManager {
    ...
    constructor(schema, observed = false) {
        ...
        this.#isSimple     = ComponentSchemaManager.#isSimpleRaw(schema);
        this.#hasReference = ComponentSchemaManager.#hasReferenceRaw(schema);

        this.#isComponentReference = 'component' in schema
        this.#isAssetReference     = 'asset' in schema;
    ...
    get isReference() {
        return this.#isComponentReference || this.#isAssetReference;
    }

    get isComponentReference() {
        return this.#isComponentReference;
    }

    get isAssetReference() {
        return this.#isAssetReference;
    }
    ...
    #serializeSimple(value) {
        if('default' in this.raw){
            if(deepEquals(value, this.raw.default)) {
                return undefined;
            }
        }
    ...
    #serializeObject(value) {
        ...
        if('default' in this.raw){
            if(deepEquals(result, this.raw.default)) {
                return undefined;
            }
        }
    ...
    #deserializeSimple(component, value, pointer = '', silent = false) {
        value ??= structuredClone(this.raw.default);
        ...
        if(this.#hasReference) {
            ...
            if(value && this.isReference) {
                let ref;
                if('component' in this.raw) {
                    ref = component.stage.references.components.create(component, { entity: value, type: this.raw.component });
                }

                if('asset' in this.raw) {
                    ref = component.stage.references.assets.create(component, { uri: value, type: this.raw.asset });
                }
    ...
    #deserializeObject(component, value, pointer = '', silent = false) {
        value ??= structuredClone(/** @type {ComponentSchemaManager<ComponentTypeSchemaObject>} */(this).raw.default);
    ...
    #deserializeArray(component, value, pointer = '', silent = false) {
        const { default: defaulValue, items } = /** @type {ComponentSchemaManager<ComponentTypeSchemaArray>} */(this).raw;
        ...
        if(value) {
            for(let i = 0, l = (Array.isArray(items) ? items.length : value.length); i < l; i++) {
     ...
     */
    copy(value) {
        if(this.#isSimple) return structuredClone(value);
    ...
    #assign(component, target, key, value, pointer = '') {
        ...
        if(this.#isSimple || target[key] == undefined || value == undefined) {
            ...
            switch(this.raw.type) {
                ...
                case 'array': {
                    const { items } = /** @type {ComponentTypeSchemaTuple|ComponentTypeSchemaArray} */(this.raw);
    ...
    static #hasReferenceRaw(schema) {
        ...
        switch(schema.type) {
            case 'object':
                for(const prop in schema.properties) {
                    if(this.#hasReferenceRaw(schema.properties[prop])) {
                        return true;
                    }
                }
                break;
     ...
     * @param {PropertyKey} key
     * @param {string} pointer
     * @return {Generator<{ schema: ComponentSchemaManager, target: any, key: PropertyKey, pointer: string }>}
     */
    * traverse(target, key, pointer = '') {
        yield { schema: this, target, key, pointer: pointer || '/' };

        if(this.#isSimple || target[key] === undefined) return;

        switch(this.raw.type) {
            case 'object': {
                for(const prop in this.#properties) {
                    yield * this.#properties[prop].schema.traverse(target[key], prop, `${pointer}/${prop}`);
                }
                break;
            }
            case 'array': {
                ...
                for(let i = 0, l = (Array.isArray(items) ? items.length : target[key].length); i < l; i++) {
                    yield * this.#items[i % this.#items.length].traverse(target[key], i, `${pointer}/${i}`);
                }
            }
        }
