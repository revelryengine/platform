...
describe('WorkerHelper', () => {
    describe('callMethod', () => {
        it('calls a method on the worker and returns the result', async () => {
            const helper = new WorkerHelper(import.meta.resolve('./__fixtures__/worker.js'), { type: 'module' });
            await helper.init();
...
describe('WorkerHelperPool', () => {
    it('loads multiple worker modules and calls their method', async () => {
        const pool = new WorkerHelperPool(import.meta.resolve('./__fixtures__/worker.js'), { count: 2, type: 'module' });
        await pool.init();
    ...
    it('aborts the method call when the provided signal is aborted', async () => {
        const helper = new WorkerHelperPool(import.meta.resolve('./__fixtures__/worker.js'), { count: 2, type: 'module' });
        await helper.init();
        const controller = new AbortController();
        const promise = helper.callMethod({ method: 'method', args: [], signal: controller.signal });
        controller.abort('test abort');
