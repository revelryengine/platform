...
import { Component } from 'revelryengine/ecs/lib/component.js';
...
/**
 * @typedef {{  
 *     prefab: { value: PrefabAsset, json: { path: string, references?: References, overrides?: Overrides } },
...
/**
 * @typedef {{[K in keyof ComponentTypes]?: ComponentJSON<ComponentTypesDefinition, K>}} ComponentTypeMapA
 */

/**
 * @template {ComponentTypesDefinition} [T = ComponentTypesDefinition]
 * @typedef {{ [K in Extract<keyof T, string>]: ComponentJSON<T, K>['value'] }} ComponentTypeValueOverrideMap
  */
/**
 * @typedef {{[key: string]: ComponentTypeValueOverrideMap }} Overrides
 * @typedef {{[key: string]: string }} References
 */

/**
 * @typedef {{ path: string, references?: References, overrides?: Overrides }} PrefabAssetValue
 * @typedef {{ meta: { name: string, description: string}, components: ComponentJSON[] }} PrefabAssetJSON
 * @typedef {PrefabAssetJSON & {
 *     entityRegExp:  RegExp;
 *     entities: {[key: string]: import('revelryengine/ecs/lib/component.js').ComponentJSONMap<ComponentTypesDefinition> };
 *     componentsWithReference: WeakSet<ComponentJSON>;
 * }} PrefabAssetData
 * @typedef {{
 *   entities: References;
    ...
    async load(signal) {
        ...
        for(const component of data.components) {
            ...
            if(component.type === 'prefab') {
                const prefab = new PrefabAsset(/** @type {ComponentJSON<ComponentTypes, 'prefab'>} */(component), [this, ...this.referer ?? []]);
    ...
    #createComponents(model) {
        if(this.stage && model.prefab.data && model.prefab.instance) {
            for(const component of model.prefab.data.components) {
                const componentData  = /** @type {ComponentJSON<ComponentTypes>} */(model.prefab.getInstanceComponentData(component));

                //stage may have other unknown component types so we need to cast it generically
                const stage = /** @type {import('revelryengine/ecs/lib/stage.js').Stage} */(/** @type {unknown} */(this.stage)); 
