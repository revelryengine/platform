...
export class SpriteAsset extends Asset {
    ...
    get key() {
        return `${super.key}#a=${this.value.alphaMode}&c=${this.value.alphaCutoff}&d=${this.value.doubleSided}&f=${encodeURIComponent(JSON.stringify(this.value.filtering))}`;
    }
        ...
        if(paths.length > 1) {
            const images = await Promise.all(paths.map((path) => {
                return new Promise((resolve, reject) => {
                    const img = new Image();
                    img.src = import.meta.resolve(path);
            ...
            mesh: new Mesh({
                primitives: [
                    new MeshPrimitive({
                        ...
                        attributes: { POSITION: geometry.accessors[1], TEXCOORD_0: geometry.accessors[2], NORMAL: geometry.accessors[3] },
