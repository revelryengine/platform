...
export class CameraModel extends Model.Typed({
    ...
    node = /**@type {import('../deps/renderer.js').ViewportCameraNode} */(new GLTFNode({ name: this.camera.name, matrix: this.transform, camera: GLTFCamera.fromJSON(this.camera, { root: {} }) }));
    ...
    reconfigure() {
        ...
        if('width' in target) {
            ...
            this.#autoResizer = new CanvasAutoResizer({
                ...
                onresize:    () => this.viewport.reconfigure(),
            });
        }
