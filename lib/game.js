 ...
 *
 * @extends {GameNode<void, import('./stage.js').Stage>}
 */
export class Game extends GameNode {
    ...
    getContext(contextName)  {
        if(contextName.includes(':')){
            ...
            const system = this.stages.getById(stageId)?.getContext(systemId.join(':'))
            if(!system) throw new Error('System context not found');
            return system;

        }
        const stage = this.stages.getById(contextName);
        if(!stage) throw new Error('Stage context not found');
        return stage;
    }
