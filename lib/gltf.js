...
export class GLTF extends GLTFProperty {
     ...
     * @param {String|URL} [uri=self.location.href] - The URI of the glTF object. Relative URIs will be based on this.
     */
    constructor(glTF = { asset: { version: '2.0', generator: 'Revelry Runtime Generation' } }, uri = self.location.href) {
        ensureSupport(glTF);
     ...
     */
    static async load(uri, signal) {
        const response = await fetch(uri, { signal });
        const buffer   = await response.arrayBuffer();
        return this.loadFromBuffer(buffer, uri, signal);
    }
    ...
    static async loadFromBuffer(buffer, uri, signal) {
        ...
        if (header[0] === MAGIC_NUMBER_BINARY_FORMAT) {
            ...
            gltf = new this(JSON.parse(new TextDecoder().decode(buffer)), uri);
        }
     ...
     */
    async load(signal) {
        await Promise.all([...extensions.values()].map(ext => ext.load?.(signal)));
