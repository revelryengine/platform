...
export class RevGameElement extends RevElement {
    ...
    */
    async loadGameFile(src) {
        ...
        try {
            const gameFileURL = import.meta.resolve(src);
            ...
            const stages = await Promise.all(Object.entries(gameFile.stages).map(([id, path]) => {
                return new Promise((resolve, reject) => {
                    const element = document.createElement('rev-stage').initProperties({
                        id,
                        src: import.meta.resolve(new URL(path, gameFileURL).toString()),
                    });

                    element.part.add('rev-stage', id);

                    element.onload  = () => resolve(element.stage);
...
export class RevStageElement extends RevElement {
    ...
    */
    ...
    constructor() {
        ...
        queueMicrotask(() => {
            this.stage = new Stage({ id: this.id, element: this });
    ...
    connectedCallback() {
        ...
        this.watch(this.stage, 'system:registered',   () => this.requestUpdate());
        this.watch(this.stage, 'system:unregistered', () => this.requestUpdate());
    }
    ...
    async loadStageFile(src) {
        ...
        try {
            ...
            const systemModules = await Promise.all(stageFile.systems.map(system => {
                return /** @type {Promise<{ bundle: Revelry.ECS.SystemBundle }>} */(import(new URL(system, stageFileURL).toString()));
            }));
            ...
            for(const { bundle } of systemModules) {
                ...
                while(search.length) {
                    const bundle = /** @type {Revelry.ECS.SystemBundle} */(search.shift());
            ...
            for(const { load, initializers, systems } of unique) {
