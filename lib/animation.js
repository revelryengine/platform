...
export class Animator {
    ...
    update(delta, loop) {
        ...
        const time = (this.loop || loop) && this.duration ? this.time % this.duration : this.time;
        ...
        for (const channel of animation.channels) {
            ...
            if(!target) continue;
            target[path] = target[path] ?? (output.type === 'SCALAR' && path !== 'weights' ? 0 : []);
             ...
             */
            if (time <= input.min[0]) {
                if(output.type === 'SCALAR' && path !== 'weights') {
