...
export class Asset extends Watchable {   
    ...
    #getFromCache() {
        const cache = _cache.create(this.load);
        cache.assets ??= new Map();
        return /** @type {AssetRef}*/(cache.assets.get(this.#path) ?? cache.assets.set(this.#path, { refs: 0 }).get(this.#path));
    }

    #deleteFromCache() {
        _cache.get(this.load)?.assets?.delete(this.#path);
    }
     ...
     */
    static getReferenceCount(path) {
        return _cache.get(this.prototype.load)?.assets?.get(path.toString())?.refs ?? 0;
    }
}