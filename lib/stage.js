 ...
 * @typedef {import('./system.js').System<T>} System
 */
...
class EntityComponentSet extends IdSet {
    /** @type {Map<Extract<keyof T, string>, Component<T>>} */
    #byType = new Map();
    ...
    add(component) {
        ...
        this.#byType.set(component.type, registered);
        return super.add(registered);
    }
    ...
    delete(component) {
        ...
        this.#byType.delete(registered.type);
        return super.delete(registered);
    }
    ...
    deleteSilent(component) {
        this.#byType.delete(component.type);
 ...
 * @extends {Watchable<{'value:change': ComponentDataMap<T>[K]['value'], 'value:notify': Map<string, any> }>}
 */
export class Component extends Watchable {
    ...
    constructor(stage, componentData) {
        ...
        this.#type     = type;
        this.#value    = /** @type {T[K]['complex']} */(stage.initializers[type]?.(componentData) ?? value);
        this.#stage    = stage;
    ...
    toJSON() {
        return { id: this.#id, entityId: this.#entityId, type: this.#type, value: this.#value };
    }
}
...
export class ComponentReference extends Watchable {
    ...
    async #resolve() {        
        ...
        if(component) {
            ...
            try {
                this.#component = (await this.#stage.waitFor(`component:add:${this.#key}`, this.#abortCtl.signal)).component;
            } catch {
                this.#aborted = true;
                this.notify('abort');
...
/** 
 * @template {ComponentTypeMap} [T = any]
 * @typedef {{ 'entity:add': { entity: Entity<T> }, 'entity:delete': { entity: Entity<T> }, [key: `entity:add:${string}`]: { entity: Entity<T> }, [key: `entity:delete:${string}`]: { entity: Entity<T> } }} EntityEvents
 */
/** 
 * @template {ComponentTypeMap} [T = any]
 * @typedef {{ 'component:add': { component: Component<T> }, 'component:delete': { component: Component<T> }, [key: `component:add:${string}`]: { component: Component<T> }, [key: `component:delete:${string}`]: { component: Component<T> } }} ComponentEvents
 */

/**
 * A stage is a collection of systems and components.
 * @template {ComponentTypeMap} [T = any]
 * @extends {GameNode<import('./game.js').Game, System<T>, SystemEvents & EntityEvents<T> & ComponentEvents<T>>}
 */
export class Stage extends GameNode {
    ...
    getComponentReference(entityId, type) {
        return /** @type {ComponentReference<T>} */ (new ComponentReference(this, entityId, type, this.#referenceCounts));
    }
 ...
 * @template [V=unknown]
 * @template [C=V]
 * @typedef {{ value: V, complex: C }} ComponentValue
 */
/**
 * @typedef {Record<string, ComponentValue>} ComponentTypeMap
 */
/**
 * @template {ComponentTypeMap} [T = any]
 * @typedef {{
 *      [K in keyof T]: T[K]['value'] | T[K]['complex']
 ...
 * ```
 * stage.initializers[keyof ComponentTypeMap] = (component: ComponentData) => ComplexComponentValue
 * ```
 * 
 * @template {ComponentTypeMap} [T = any]
 * @typedef {Partial<{
 *     [K in keyof T]: (c: ComponentDataMap<T>[K]) => T[K]['complex'];
