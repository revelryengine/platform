...
class Component extends Watchable {
    #value;

    constructor({ id, type, entityId, value }) {
        super();
        this.id       = id;
        this.type     = type;
...
export class Stage extends GameNode {
    ...
    #registerSystem(system) {
        ...
        for (const { model: ModelClass } of Object.values(SystemClass.models)) {
            for (const { type, initializer } of Object.values(ModelClass.components)) {
                ...
                if(initializer) this.#initializers.set(type, initializer);
            }
        }
    }
    ...
    #unregisterSystem(system) {
        ...
        for (const { model: ModelClass } of Object.values(SystemClass.models)) {
            for (const { type, initializer } of Object.values(ModelClass.components)) {
                ...
                if(initializer && this.#initializers.get(type) === initializer) this.#initializers.delete(type);
            }
        }
    }

    #registerComponent(component) {
        const initializer = this.#initializers.get(component.type);
        if(initializer && !(component.value instanceof initializer)) component.value = new initializer(component.value);

        if(!(component instanceof Component)) component = new Component(component);
