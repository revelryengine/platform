 ...
 * 
 * @typedef  {object}           ResolvedWatchOptions
 * @property {WatchHandler}     handler   - A handler function
 * @property {string|symbol}    type      - The change type to watch for. Defaults to all types.
...
export class Watchable {
     ...
     */
    watch(options) {
        const { handler, type, immediate, signal } = Watchable.resolveWatchOptions(options);
    ...
    unwatch(options) {
        ...
        const { handler, type, immediate } = Watchable.resolveWatchOptions(options);
     ...
     * @example ```const [arg1, arg2] = await watchable.waitFor('example');```
     * @param {string|symbol}    type     - The change type to watch for. Defaults to all types.
    ...
    waitFor(type, signal) {
        return new Promise((resolve, reject) => {
            ...
            this.watch({ type, signal: abortCtl.signal, handler: (...args) => {
                resolve(args);
                abortCtl.abort();
            } });
        });
     ...
     * @returns {ResolvedWatchOptions}
     */
    static resolveWatchOptions(options) {
