...
export class Watchable {
     ...
     * @overload
     * @param {WildCardImmediateHandler<T>} handler
     * @param {undefined} [options]
     * @return {void}
     * 
     * Watch for all events and dispatch immediately.
     * @overload
     * @param {WildcardDeferredOptions<T>} options  
     * @return {void}
     *
     * Watch for all events
     * @overload
    ...
    watch(anyType, anyOptions) {
        ...
        const { type, handler, deferred, signal, once } = /** @type {OptionsResolved<T>} */(options);
    ...
    unwatch(anyType, anyOptions) {
        ...
        const { type, handler, deferred } = /** @type {OptionsResolved<T>} */(options);
    ...
    waitFor(type, signal) {
        return new Promise((resolve, reject) => {
            ...
            this.watch(type, { signal, once: true, handler: resolve });
        });
    }
    ...
    #createOnceHandler(type, deferred, handler) {       
        ...
        const cache = /** @type {State<T>} */(this.#state).handlers.once.create(handler);
        if(deferred) {
            wrapped = /** @type {U} */((...args) => {
                handler(...args);
    ...
    #getOnceHandler(type, deferred, handler) {
        const cache = /** @type {State<T>} */(this.#state).handlers.once.get(handler);
        if(cache) {
            if(deferred) {
...
/**
 * @typedef {Record<string, unknown>} EventMap
 */
/**
 * @template {EventMap} T
 * @template {keyof T} K
...
/**
 * @template {EventMap} T
 * @typedef {(data: Map<keyof T, T[keyof T]>) => void} WildCardDeferredHandler
 */
/**
 * @template {EventMap} T
 ...
 * @template {EventMap} T
 * @typedef {{
 *     handler:    WildCardDeferredHandler<T>;
 *     deferred:   true;
 *     once?:      boolean;
 *     signal?:    AbortSignal;
 * }} WildcardDeferredOptions
 */

/**
 * @template {EventMap} [T = EventMap]
 * @typedef {keyof T | WildCardImmediateHandler<T> | WildcardDeferredOptions<T> | WildcardImmediateOptions<T>} AnyType
 */
/**
 * @template {EventMap} [T = EventMap]
 * @template {keyof T} [K = keyof T]
