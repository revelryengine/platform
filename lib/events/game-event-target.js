 ...
 * @property {boolean} [capture=false] - A Boolean indicating that events of this type will be dispatched to the
 * registered listener before being dispatched to any GameEventTarget beneath it in the GameNode tree.
 * @property {Boolean|Promise<any>} [once=false] - If a Boolean is provided and is true the listener would be
...
export class GameEventTarget {
    ...
    /**
     * @type {SetMap<String, Function>}
     * @desc All bubble phase listeners mapped by type
     */
    #eventBubbleListeners = new SetMap();

    /**
     * @type {SetMap<String, Function>}
     * @desc All capture phase listeners mapped by type
     */
    #eventCaptureListeners = new SetMap();

    /**
     * @type {GameEventTarget|undefined}
     * @desc The parent GameEventTarget
     */
    parent = undefined;
     ...
     * @param {(e: GameEvent) => void} listener - The Function to call when event type is dispatched.
    ...
    addEventListener(type, listener, options = {}) {
        ...
        const once    = (typeof options === "boolean") ? false : options.once;

        const listeners = capture ? this.#eventCaptureListeners : this.#eventBubbleListeners;
        const stack     = listeners.add(type, listener);

        if (once) {
            if (once instanceof Promise) {
                once.then(() => {
    ...
    dispatchEvent(event) {
        if (event.eventPhase === GameEvent.NONE && event.bubbles) {
            let node = /** @type {GameEventTarget} */(this);
            while (node.parent) {
    ...
    async awaitEvent(type) {
        return new Promise((resolve) => {
            this.addEventListener(type, (gameEvent) => resolve(gameEvent), { once: true });
        });
    }
}
