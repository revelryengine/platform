...
function worker() {
    ...
    const dracoPromise = new Promise(async (resolve) => {
        const { DracoDecoderModule} = await importUMD(DRACO_DECODER_URL);
...
const workerHelper = new WorkerHelper({ worker, count:  4, constants: `const DRACO_DECODER_URL = '${DRACO_DECODER_URL}';` })
