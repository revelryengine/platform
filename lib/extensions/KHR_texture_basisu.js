...
export class KHRTextureBasisuTexture extends GLTFProperty {
     ...
     * @param {AbortController} [abortCtl] - The abort controller for the given transode request
     */
    async transcode(supportedCompression, abortCtl) {
        const arrayBuffer = this.source.getImageData();
        const response = await workerHelper.postMessage({ type: 'transcode', arrayBuffer, supportedCompression }, abortCtl);
