...
export class KHRMaterialsVariantsVariant extends GLTFProperty {
    ...
    /**
     * @type {Boolean} - The runtime state of variant. Set this to true to activate the material.
     */
    get active() {
        return this.#active;
    }
...
export class KHRMaterialsVariantsPrimitive extends GLTFProperty {
    ...
    /**
     * Get the active material for a given primitive.
     * @returns {Material}
     */
    getActiveMaterial(primitive) {
        const activeMaterial = this.mappings.find(mapping => {
            return mapping.variants.some(variant => variant.active);
        });
