...
export class Watchable {
    ...
    waitFor(type, signal) {
        return new Promise((resolve, reject) => {
            ...
            this.watch(type, { signal, once: true, handler: /** @type {WatchHandler<T, keyof T>} */(resolve) });
        });
    }
     ...
     */
    isWatched(type) {
        return !!(this.#state?.handlers.deferred.has(type ?? WILDCARD) || this.#state?.handlers.immediate.has(type ?? WILDCARD));
    }
     ...
     */
    isQueued(type) {
        return !!this.#state?.queued.has(type ?? WILDCARD);
    }
     ...
     * @param {{ type?: keyof T, deferred?: boolean, handler: U }} options
     */
