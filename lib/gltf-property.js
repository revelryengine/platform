...
export class GLTFProperty {
    ...
    dereference(root, parent) {
        for(const { name, type, collection, alias, location = 'root', assign = {} } of this.constructor.referenceFields) {
            ...
            switch(type) {
                ...
                case 'json': { //json pointer
                    const tokens = this[name].substring(1).split(/\//);
                    const [collection, index] = tokens;
                    const rootTarget = { collection, target: root[collection][index]};

                    let obj = root;
                    for (let i = 0; i < tokens.length - 1; i++) {
                        obj = obj[tokens[i]];
                    }
                    this[name] = { target: obj, path: tokens.at(-1), rootTarget };
                }
            }
        }
    ...
    ensureReferences(root, collection) {
        if(collection.indexOf(this) === -1) collection.push(this);
...
async load(abortCtl) {
    return await Promise.all(Object.values(this.extensions).map(ext => {
        return ext.load?.(abortCtl);
    }));
}

async loadOnce(abortCtl) {
    return await (pending.get(this) || pending.set(this, this.load(abortCtl)).get(this));
}
